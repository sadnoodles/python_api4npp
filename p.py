...
#! /usr/bin/env python
# G. Gerebtzoff
#I modified the generate_python_autocomplete.py .so can output some useful data.
"""
...
def generate_python_autocomplete(python_script, **keywords):
        ...
        for key, val in imports.iteritems():
            ...
            if process == True:
                ...
                if key.find("$") == -1: # found in some lucene functions
                    try:
                        current_function = eval(key) #获取函数或模块
                    except:
                        pass
                if current_function:
                    docs = ""
                    try:
                        doc = getdoc(current_function) #获取文档
                        if doc is not None:
                            docs = doc.strip()
                        ...
                        pass
                    if isroutine(current_function): #如果是非内置函数
                        argspec = None  #参数
                        try:
                            argspec = getargspec(current_function) #获取参数
                            args = argspec[0] #参数
                            keywords = argspec[2] #键值对
                                ...
                                for k in key.split("."):
                                    if k not in outputs:
                                        if counter == 0:
                                            ...
                                            doc = ("Function of %s" % ".".join(key.split(".")[0:-1]))
                                        else:
                                            doc = ""
                                        doc
                                        outputs[k] = [[], (), doc]
                        ...
                        doc = ("(%s)" % type(current_function).__name__)
                        outputs[key] = [[], (), doc]
        imports = new_imports
    if no_builtin == False:
    ...
    # fff=open('a.py','w')
    # fff.write('bigdict={\n')
    # for i,j in outputs.items():
        # fff.write(repr(i)+':'+repr(j)+',\n')
    # fff.write('\n}')
    # fff.close()
    keys = outputs.keys()
    keys.sort()
